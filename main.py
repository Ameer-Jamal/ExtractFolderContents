...
def get_code_context(folder_path):
    """
    Traverses a folder, reads text-based code files, and formats them for AI context.
    ...
    Returns:
        str: A string containing the formatted file contents with headers,
             ready to be pasted into an AI prompt.
    """
    output_text = ""
    excluded_dirs = {".git", "node_modules", "venv", "__pycache__"}  # Add any directories to exclude here
    valid_extensions = {".py", ".js", ".html", ".css", ".txt", ".json", ".md", ".java", ".c", ".cpp", ".go", ".sh"} # Add or modify extensions as needed
    ...
    for root, dirs, files in os.walk(folder_path):
        ...
        for file in files:
            if os.path.splitext(file)[1].lower() in valid_extensions: # Check file extension
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as f: # Explicitly open in text mode with utf-8 encoding
                        file_content = f.read()
                        output_text += f"\n\n--- FILE: {file_path} ---\n\n{file_content}"
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(folder_to_copy):
        ...
        ai_context_text = get_code_context(folder_to_copy)
        if ai_context_text:
            pyperclip.copy(ai_context_text)  # Copy the output to the clipboard
            print("\n--- Copied Content (and automatically copied to clipboard!) ---\n") # Modified message
            print(ai_context_text)
            print("\n--- End of Copied Content ---")
            print("\nThe content has been automatically copied to your clipboard and is ready to be pasted into your AI prompt.") # Added confirmation
        else:
